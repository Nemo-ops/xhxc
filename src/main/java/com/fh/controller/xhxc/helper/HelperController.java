package com.fh.controller.xhxc.helper;
...
public class HelperController extends BaseController {
	...
	String menuUrl = "helper/listUsers.do"; //菜单地址(权限用)
	...
	public ModelAndView listUsers(Page page)throws Exception{
		...
		page.setPd(pd);
		List<PageData>	userList = userService.listHelpers(page);	//列出用户列表
		pd.put("ROLE_ID", "1");
		List<Role> roleList = roleService.listAllRolesByPId(pd);//列出所有系统用户角色
		mv.setViewName("xhxc/helper/helper_list");
	...
	public ModelAndView goAddU()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("xhxc/helper/helper_edit");
