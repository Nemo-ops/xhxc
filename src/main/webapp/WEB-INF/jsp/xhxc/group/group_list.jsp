...
<script type="text/javascript">
    ...
    function delUser(userId,msg){
        bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
            if(result) {
                top.jzts();
                var url = "<%=basePath%>group/deleteU.do?USER_ID="+userId+"&tm="+new Date().getTime();
    ...
    function add(){
        ...
        diag.Drag=true;
        diag.Title ="新增";
        diag.URL = '<%=basePath%>group/goAddU.do';
        diag.Width = 469;
        diag.Height = 510;
    ...
    function editUser(user_id){
        ...
        diag.Drag=true;
        diag.Title ="资料";
        diag.URL = '<%=basePath%>group/goEditU.do?USER_ID='+user_id;
        diag.Width = 469;
        diag.Height = 510;
        diag.CancelEvent = function(){ //关闭事件
            if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
            ...
            if(result) {
                ...
                if(str==''){
                    ...
                    if(msg == '确定要删除选中的数据吗?'){
                        ...
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>group/deleteAllU.do?tm='+new Date().getTime(),
                            data: {USER_IDS:str},
                            dataType:'json',
    ...
    function toExcel(){
        ...
        window.location.href='<%=basePath%>group/excel.do?keywords='+keywords+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&ROLE_ID='+ROLE_ID;
    }
    ...
    function fromExcel(){
        ...
        diag.URL = '<%=basePath%>group/goUploadExcel.do';
        diag.Width = 300;
        diag.Height = 150;
        diag.CancelEvent = function(){ //关闭事件
            if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
