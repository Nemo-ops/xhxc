			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="school/listUsers.do" method="post" name="userForm" id="userForm">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center">校名</th>
									<th class="center">所在省/自治区/直辖市</th>
									<th class="center">所在地（市/县）</th>
								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										...
										<tr>
											...
											<td class="center"><img  style="cursor:default;height:50px;" src="${NginxUrl}/${user.PHOTO}"/></td>
...
function delUser(userId,msg){
	bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=basePath%>school/deleteU.do?USER_ID="+userId+"&tm="+new Date().getTime();
...
function add(){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>school/goAddU.do';
	 diag.Width = 490;
	 diag.Height = 550;
...
function editUser(user_id){
	 ...
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '<%=basePath%>school/goEditU.do?USER_ID='+user_id;
	 diag.Width = 490;
	 diag.Height = 550;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		if(result) {
			...
			if(str==''){
				...
				if(msg == '确定要删除选中的数据吗?'){
					...
					$.ajax({
						type: "POST",
						url: '<%=basePath%>school/deleteAllU.do?tm='+new Date().getTime(),
				    	data: {USER_IDS:str},
						dataType:'json',
...
function toExcel(){
	...
	window.location.href='<%=basePath%>school/excel.do?keywords='+keywords+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&ROLE_ID='+ROLE_ID;
}
...
function fromExcel(){
	 ...
	 diag.URL = '<%=basePath%>school/goUploadExcel.do';
	 diag.Width = 300;
	 diag.Height = 150;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function viewUser(USERNAME){
	if('admin' == USERNAME){
	 ...
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '<%=basePath%>school/view.do?USERNAME='+USERNAME;
	 diag.Width = 469;
	 diag.Height = 380;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
